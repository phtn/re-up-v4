...
import { QGrid } from "@@ui/grid";
import { DarkTouch } from "@@ui/touch";
import { onError, onSuccess, signinToast } from "@@utils/toast";
...
export const Login = ({
  ...
  const [authed] = useAuthState(auth);
  ...
  const [signInWithEmailAndPassword, userSignin, loading, error] =
  ...
  const isAuthed = authed?.uid !== null && authed?.uid !== undefined;

  const loadster = useCallback(() => {
    signinToast({
      b: isAuthed,
    });
  }, [isAuthed]);
