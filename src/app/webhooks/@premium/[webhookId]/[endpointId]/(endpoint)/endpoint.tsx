...
import { type EndpointInfoSchema } from "@src/server/resource/endpoint";
...
export const DetailContent = memo(({ endpointId }: DetailContentProps) => {
  ...
  const { getEndpointById } = useFirebaseWebhook({ app_id: webhookId });

  const endpointData: EndpointInfoSchema | undefined = useMemo(() => {
    return getEndpointById(endpointId);
  }, [endpointId, getEndpointById]);
